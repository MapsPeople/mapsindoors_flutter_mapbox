...
class MPPolygon extends MPGeometry {
  ...
  static MPPolygon? fromJson(json) => json != null && json != "null" ? MPPolygon._fromJson(json is String ? jsonDecode(json) : json) : null;
  ...
  List<List<MPPoint>> _calcPoints() {
    ...
    for (var coordSet in coordinates) {
      ...
      for (var pair in coordSet) {
        pointsList.add(MPPoint.withCoordinates(latitude: pair[MPGeometry._lat], longitude: pair[MPGeometry._lng]));
      }
  ...
  MPBounds _calcBounds() {
    if (bbox.isNotEmpty) {
      ...
      for (var p in coordinates[0]) {
        builder.include(MPPoint.withCoordinates(latitude: p[MPGeometry._lat], longitude: p[MPGeometry._lng]));
      }
