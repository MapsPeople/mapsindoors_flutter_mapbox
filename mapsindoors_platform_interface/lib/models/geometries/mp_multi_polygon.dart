...
class MPMultiPolygon extends MPGeometry {
  ...
  List<num> _bbox = List.empty(growable: true); // 0 = Lng W, 1 = Lat S, 2 = Lng E, 3 = Lat N
  num? _area;
  MPBounds? _bounds;
  ...
  static MPMultiPolygon? fromJson(json) => json != null && json != "null" ? MPMultiPolygon._fromJson(json is String ? jsonDecode(json) : json) : null;

  MPMultiPolygon._fromJson(data) {
    if (data is List<dynamic>) {
      coordinates = convertJson4dArray<num>(data);
      ...
      if (data['bbox'] is Map) {
        bbox = [data['bbox']["southwest"][0], data['bbox']["southwest"][1], data['bbox']["northeast"][0], data['bbox']["northeast"][1]];
      } else {
  ...
  MPBounds _calcBounds() {
    if (bbox.isNotEmpty) {
      return MPBounds(northeast: MPPoint.withCoordinates(latitude: bbox[3], longitude: bbox[2]), southwest: MPPoint.withCoordinates(latitude: bbox[1], longitude: bbox[0]));
    } else {
      MPBoundsBuilder builder = MPBounds.builder();
      for (var p in coordinates) {
        for (var outline in p[0]) {
          builder.include(MPPoint.withCoordinates(latitude: outline[MPGeometry._lat], longitude: outline[MPGeometry._lng]));
        }
      }
  ...
  Future<num?> getSquaredDistanceToClosestEdge(MPPoint point) async => await UtilPlatform.instance.geometryGetSquaredDistanceToClosestEdge(this, point);
  ...
  @override
  Future<num?> get area async => _area ??= await UtilPlatform.instance.geometryArea(this);
  ...
  @override
  Map<String, dynamic> toJson() => {"coordinates": coordinates, "bbox": bbox, "type": type};
