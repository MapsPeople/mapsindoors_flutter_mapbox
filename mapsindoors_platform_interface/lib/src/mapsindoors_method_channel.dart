...
class MethodChannelMapsindoors extends MapsindoorsPlatform {
  ...
  @visibleForTesting
  final mapsIndoorsMethodChannel = const MethodChannel('MapsIndoorsMethodChannel');
  final listenerChannel = const MethodChannel('MapsIndoorsListenerChannel');
  final List<OnMapsIndoorsReadyListener> _readyListeners = List.empty(growable: true);
  ...
  Future<MPError?> load(String key) async {
    final error = await mapsIndoorsMethodChannel.invokeMethod('MIN_initialize', {"key": key});
    return MPError.fromJson(error);
  }

  @override
  Future<bool?> locationDisplayRuleExists(MPLocationId location) async {
    return await mapsIndoorsMethodChannel.invokeMethod("MIN_locationDisplayRuleExists", {"id": location.value});
  }

  @override
  Future<bool?> displayRuleNameExists(String name) async {
    return await mapsIndoorsMethodChannel.invokeMethod("MIN_displayRuleNameExists", {"name": name});
  }
  ...
  Future<bool?> checkOfflineDataAvailability() {
    return mapsIndoorsMethodChannel.invokeMethod("MIN_checkOfflineDataAvailability");
  }
  ...
  Future<void> disableEventLogging(bool disable) {
    return mapsIndoorsMethodChannel.invokeMethod("MIN_disableEventLogging", {"disable": disable});
  }
  ...
  Future<List<String>?> getAvailableLanguages() {
    return mapsIndoorsMethodChannel.invokeListMethod<String>("MIN_getAvailableLanguages");
  }

  @override
  Future<MPBuildingCollection?> getBuildings() async {
    final buildings = await mapsIndoorsMethodChannel.invokeMethod("MIN_getBuildings");
    return MPBuildingCollection.fromJson(buildings);
  }

  @override
  Future<MPCategoryCollection?> getCategories() async {
    final categories = await mapsIndoorsMethodChannel.invokeMethod("MIN_getCategories");
  ...
  Future<MPLocation?> getLocationById(String id) async {
    final location = await mapsIndoorsMethodChannel.invokeMethod("MIN_getLocationById", {"id": id});
    return MPLocation.fromJson(location);
  }

  @override
  Future<List<MPLocation>?> getLocations() async {
    final locations = await mapsIndoorsMethodChannel.invokeMethod("MIN_getLocations");
    return convertMIList<MPLocation>(jsonDecode(locations), (p0) => MPLocation.fromJson(p0));
  }

  @override
  Future<List<MPLocation>?> getLocationsByExternalIds(List<String> ids) async {
    final locations = await mapsIndoorsMethodChannel.invokeMethod("MIN_getLocationsByExternalIds", {"ids": ids});
    return convertMIList<MPLocation>(jsonDecode(locations), (p0) => MPLocation.fromJson(p0));
  }

  @override
  Future<List<MPLocation>?> getLocationsByQuery(MPQuery? query, MPFilter? filter) async {
    final locations = await mapsIndoorsMethodChannel.invokeMethod("MIN_getLocationsByQuery", {"query": query?._jsonEncode(), "filter": filter?._jsonEncode()});
    return convertMIList<MPLocation>(jsonDecode(locations), (p0) => MPLocation.fromJson(p0));
  }

  @override
  Future<List<MPMapStyle>?> getMapStyles() async {
    final mapStyles = await mapsIndoorsMethodChannel.invokeMethod("MIN_getMapStyles");
    return convertMIList<MPMapStyle>(jsonDecode(mapStyles), (p0) => MPMapStyle.fromJson(p0));
  }
  ...
  void setPositionProvider(MPPositionProviderInterface? provider) {
    ...
    _positionProvider?.addOnPositionUpdateListener(onPositionUpdate);
    mapsIndoorsMethodChannel.invokeMethod("MIN_setPositionProvider", {"remove": provider == null, "name": provider?.name});
  }
  ...
  Future<MPError?> synchronizeContent() async {
    final error = await mapsIndoorsMethodChannel.invokeMethod("MIN_synchronizeContent");
    return MPError.fromJson(error);
  }

  @override
  Future<void> applyUserRoles(List<MPUserRole> userRoles) {
    return mapsIndoorsMethodChannel.invokeMethod("MIN_applyUserRoles", {"userRoles": convertMIListToJson(userRoles)});
  }

  @override
  Future<List<MPUserRole>?> getAppliedUserRoles() async {
    final userRoles = await mapsIndoorsMethodChannel.invokeMethod("MIN_getAppliedUserRoles");
    return convertMIList<MPUserRole>(jsonDecode(userRoles), (p0) => MPUserRole.fromJson(p0));
  }

  @override
  Future<MPUserRoleCollection?> getUserRoles() async {
    final userRoles = await mapsIndoorsMethodChannel.invokeMethod("MIN_getUserRoles");
    return MPUserRoleCollection.fromJson(userRoles);
  }

  @override
  Future<MPGeocodeResult?> reverseGeoCode(MPPoint point) async {
    final geoCode = await mapsIndoorsMethodChannel.invokeMethod("MIN_reverseGeoCode", {"point": point._jsonEncode()});
