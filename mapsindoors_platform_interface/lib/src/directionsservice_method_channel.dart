...
class MethodChannelDirectionsService extends DirectionsServicePlatform {
  final directionsServiceMethodChannel = const MethodChannel('DirectionsServiceMethodChannel');
  ...
  Future<MPRoute> getRoute(MPPoint origin, MPPoint destination) async {
    Map<String, dynamic>? result = await directionsServiceMethodChannel.invokeMapMethod('DSE_getRoute', {'origin': origin._jsonEncode(), 'destination': destination._jsonEncode()});
    ...
    if (route != null) {
      ...
      return Future.error(MPError(code: MPError.unknownError, message: "Did not receieve route object from Platform"));
    }
  }
